 ...
 * str이 한글 자음인지 확인합니다.
 * @example
 * Hanguler.isConsonant('ㄱ'); // true
 * Hanguler.isConsonant('ㅏ'); // false
 * Hanguler.isConsonant('a'); // false
 * @param {string} str
 * @returns {boolean}
 */
 ...
 * str이 한글 모음인지 확인합니다.
 * @example
 * Hanguler.isVowel('ㄱ'); // false
 * Hanguler.isVowel('ㅏ'); // true
 * Hanguler.isVowel('a'); // false
 * @param {string} str
 * @returns {boolean}
 */
 ...
 * @example
 * Hanguler.isChoseong('ㄱ'); // true
 * Hanguler.isChoseong('ㄳ'); // false
 * Hanguler.isChoseong('ㅏ'); // false
 * Hanguler.isChoseong('a'); // false
 * @param {string} str
 * @returns {boolean}
 */
 ...
 * @example
 * Hanguler.isJungseong('ㄱ'); // false
 * Hanguler.isJungseong('ㄳ'); // false
 * Hanguler.isJungseong('ㅏ'); // true
 * Hanguler.isJungseong('a'); // false
 * @param {string} str
 * @returns {boolean}
 */
 ...
 * @example
 * Hanguler.isJongseong('ㄱ'); // true
 * Hanguler.isJongseong('ㄳ'); // true
 * Hanguler.isJongseong('ㅏ'); // false
 * Hanguler.isJongseong('a'); // false
 * @param {string} str
 * @returns {boolean}
 */
 ...
 * str이 한글인지 확인합니다.
 * @example
 * Hanguler.isHangul('ㄱ'); // true
 * Hanguler.isHangul('ㅏ'); // true
 * Hanguler.isHangul('가'); // true
 * Hanguler.isHangul('a'); // false
 * @param {string} str
 * @returns {boolean}
 */
 ...
 * str이 완전한 한글인지 확인합니다.
 * @example
 * Hanguler.isHangulComplete('ㄱ'); // false
 * Hanguler.isHangulComplete('ㅏ'); // false
 * Hanguler.isHangulComplete('가'); // true
 * Hanguler.isHangulComplete('a'); // false
 * @param {string} str
 * @returns {boolean}
 */
  ...
  const str = arr.map((current, index, array) => {
    ...
    if (isHangul(current)) {
      ...
      if (isConsonant(current) && isVowel(next)) {
        if ((isConsonant(next2) && (isConsonant(next3) || !isVowel(next3) || !next3))) {
          accumulator.push([current, next, next2]);
          accumulator.push(null);
          accumulator.push(null);
        } else {
          accumulator.push([current, next]);
          accumulator.push(null);
        }
      } else if (accumulator[accumulator.length - 1] === null) {
        accumulator.pop();
      } else {
 ...
 * 음절을 분해하여 반환합니다.
 * @example
 * Hanguler.flatDisassemble('너 때문에 흥이 다 깨져버렸으니까 책임져');
 * // [
 * //  'ㄴ', 'ㅓ', ' ', 'ㄸ', 'ㅐ', 'ㅁ', 'ㅜ', 'ㄴ', 'ㅇ', 'ㅔ', ' ',
 * //  'ㅎ', 'ㅡ', 'ㅇ', 'ㅇ', 'ㅣ', ' ', 'ㄷ', 'ㅏ', ' ',
 ...
 * @example
 * Hanguler.endsWithConsonant('수소'); // false
 * Hanguler.endsWithConsonant('헬륨'); // true
 * @param {string} str
 * @returns {boolean}
 */
...
/**
 * strA와 strB의 음운이 같은지 확인합니다.
 * @example
 * Hanguler.equals('흑ㅠ', '흐규'); // true
 * @param {string} strA
 * @param {string} strA
 * @returns {boolean}
 */
const equals = (strA, strB) => flatDisassemble(strA).join('') === flatDisassemble(strB).join('');

/**
 * target에 search가 포함되는지 확인합니다.
 * @example
 * Hanguler.includes('달걀', '닭'); // true
 * @param {string} target
 * @param {string} search
 * @returns {boolean}
 */
